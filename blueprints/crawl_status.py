...
(08/2026, xem lịch sử trao đổi). Cho admin thấy nhanh bảng company đang
...
`crawl_bp` từ blueprints.crawl nên mới vướng vòng lặp — file này thì
không).

Nguồn dữ liệu: crawler_client.list_all_companies() (đã tự phân trang
sẵn, dùng đúng pattern dashboard.py đang dùng để tính thống kê) + hàm
đếm crawler_client.company_field_health() — KHÔNG cần thêm gì phía
backend thật (scrap-jd-api), toàn bộ tính on-the-fly ở đây."""
